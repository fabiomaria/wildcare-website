...
`datenschutz.html` has no DE/EN toggle at all (no `data-de`/`data-en` attributes, no `i18n.js` on the page) — it is German-only, matching how the other legal page (`impressum.html`) is also German-only. Add a new German-only subsection under "4. Eingebundene Dienste" (alongside the existing Google Maps / Google Fonts / Meta Pixel / Tally subsections there), describing the new data collection: what's collected (E-Mail-Adresse + Einwilligung), why (Montags-Erinnerung), where it's stored (Notion, via einen Cloudflare Worker als Vermittler), legal basis (Art. 6 Abs. 1 lit. a DSGVO — Einwilligung, consistent with how the Meta Pixel subsection above cites its legal basis), and how to withdraw consent / request deletion (e.g. per E-Mail an hello@wildcare.space).
