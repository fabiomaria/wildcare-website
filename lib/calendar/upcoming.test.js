...
test("recurring mode: normal session uses the default teacher", () => {
  const def = montagskursDef({ defaultTeacher: "Verena" });
  const statusById = new Map([["montagskurs", { status: "published", includable: true }]]);
  const result = selectUpcomingItems([def], { nowLocal: "2026-08-01T00:00", statusById });
  assert.equal(result[0].occurrence.teacher, "Verena");
});
