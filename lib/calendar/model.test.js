...
test("toDefinition derives public routes from the stable slug", () => {
  const workshop = JSON.parse(JSON.stringify(DATES_RECORD));
  delete workshop.global.route;
  assert.equal(toDefinition(workshop, { venues: VENUES }).route, "/how-we-move-together");

  const event = JSON.parse(JSON.stringify(workshop));
  event.global.id = "garden-jam";
  event.global.event_type = "jam";
  assert.equal(toDefinition(event, { venues: VENUES }).route, "/events/garden-jam");
});
