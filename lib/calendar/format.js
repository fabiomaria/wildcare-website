...
function formatDate(startLocal, locale, { weekday = false } = {}) {
  const dtf = new Intl.DateTimeFormat(LOCALE_TAG[locale], {
    weekday: weekday ? "long" : undefined,
    day: "numeric", month: "long", year: "numeric", timeZone: "UTC",
  });
  return dtf.format(toUtcDate(startLocal));
}
...
function formatDateRange(startLocal, endLocal, locale) {
  ...
  // Intl uses locale-specific spacing around the en dash. The site copy and
  // existing parity contract use a compact range in English.
  return dtf.formatRange(a, b).replace(/\u2009–\u2009/g, "–");
}
...
module.exports = { formatDate, formatDateRange, formatRecurring, formatNextOccurrence };
