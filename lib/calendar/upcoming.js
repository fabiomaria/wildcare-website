...
function selectUpcomingItems(definitions, { nowLocal, statusById, limit = 6 }) {
  ...
  for (const def of definitions) {
    ...
    if (def.mode === "dates") {
      ...
      if (!next) continue;
      const override = (def.overrides || def.featured || []).find((item) => item.id === next.date);
      const occurrence = {
        ...next,
        teacher: override?.teacher || def.defaultTeacher || null,
      };
      candidates.push({
        def,
        kind: override ? "featured" : "montagskurs",
        occurrence,
